...
function loadRateDescDetails(rateDescId) {
		...
		self.opener.loadRateDescToUpdate(listForCondition);
	     window.close();
	}
...
function setDefaultForConditionSearch(){
		...
		$("#SOURCE_TARIFF").val(unescape($(document).getUrlParam("sourceCode")));
		$("#GROUP_NAME").val(unescape($(document).getUrlParam("groupCode")));
