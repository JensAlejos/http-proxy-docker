...
enforceSecurityDivAndButtons( "savecontacts", isprintfreightbillSavensend);
enforceSecurityDivAndButtons( "sendself", isprintfreightbillSavensend);
enforceSecurityDivAndButtons( "sendpol", isprintfreightbillSavensend);
...
enforceSecurityDivAndButtons( "watermark", isprintfreightbillWatermark);
...
if(isprintfreightbillCommentDisplayOnly) {
...
$('#printFreightBillForm').validationEngine('attach');
...
$('#shipmentSequenceNumber').gatesAutocomplete({
 ...
 $('#customerName').gatesAutocomplete({
	...
	//D026114:  "FB"  SHOULD APPEAR IN THE DROP-DOWN ONLY WHEN THE SHIPMENT STATUS = 'RATD', 'INPR', 'CORR', 'ISSD'
	// Removed, this was not a good solution, moved to PrintFreightBillViewHelper
	...
	$('#selectedDocType').change(function(){
		...
		{ 
			...
			// D027318, DISABLE EDI for everything but FB
			if($('#selectedDocType').val()=='FB') {
				if($('#ediBtn').val()=="false"){
	...
	$('#selectedContactId').change(function(){
		...
		if($('#selectedContactId').val()!='-1')
		{
			...
			$('#printFreightBillForm').attr('action','addToGrid/addPrimary');
			$('#printFreightBillForm').ajaxSubmit(options);
		}
	});
	...
	$('#ratedRadio').change(function(){
		...
		if($('#selectedDocType').val()!=-1) {
			...
			$.ajax({
				async: true,
				type : "GET",
				url : _context + "/printFreightBill/getRatedAndCopies",
				data : {
					rated :'true'
				},
				success : function(responseText) {
					...
					else if($('#noOfCopies').val().trim() <= 0 || $('#noOfCopies').val().trim()=="-32768" ) { 	
...
$('#unRatedRadio').change(function(){
		...
		if($('#selectedDocType').val()!=-1) {
			...
			$.ajax({
				...
				success : function(responseText) {
					...
					else if($('#noOfCopies').val().trim()=="-32768") { 
							/*
							 * -32768 value is minimum short value which has been used in no of copies 
							 * when no print options are found but the print has to be allowed
							*/
	...
	if($('#selectedDocType').val()!="-1" && $('#selectedDocType').val()!=null && $('#selectedDocType').val()!=undefined
		...
		// D030422 always populate default
	...
	$('#sendpol').click(function(){	
		$('#isSaveOnly').val('N');
		$('#sendPOL').val('Y');
		saveandsend(false);
	});
	$('#sendself').click(function(){
		$('#isSaveOnly').val('N');
		$('#sendPOL').val('N');
		saveandsend(true);
	});
	...
	$('#bill').click(function() {
		blockUI();
		makeGridForReError("printFreightBill");
		rateBill();
	});
	...
	$('#print').click(function(){	
		// document.location.href='${pageContext.servletContext.contextPath}/printFreightBill/go?shipmentNumber='+
		...
		if($('#selectedDocType').val()!="-1"){
			...
			console.log("For Print Bill status:"+$('#statusCode').val()+", selectedDocType:"+$('#selectedDocType').val());
			if($('#selectedDocType').val()=="FB" && $('#statusCode').val()!="ISSD" && $('#statusCode').val()!="CORR"){
				var c = confirm("The B/L is not in ISSD/CORR status are you sure you want to produce this doc?");
				if (c == false) {
					return;
				}
			}
			...
			/*var url='../printFreightBill/printPDF/?docType='+$('#selectedDocType').val()+'&partyCode='+$('#selectedPartyType').val()
			...
			$('#printFreightBillForm').attr('action','printPDF');
			$('#printFreightBillForm').attr('method','POST');
			blockUI();
			$('#printFreightBillForm').ajaxSubmit(optionsForAjax);
	...
	$('#web').click(function(){	
		...
		/*
		if($('#selectedPartyType').val()=="-1") {
			 $('#selectedPartyType').validationEngine('showPrompt', 'Please Select Party', 'error', 'topRight', true);
			 return;
		 }*/
		...
		if($('#selectedDocType').val()!="-1"){
			...
			$('#printFreightBillForm').attr('action','web');
			$('#printFreightBillForm').attr('method','POST');
			blockUI()
			$('#printFreightBillForm').ajaxSubmit(optionsForAjax);
	...
	$('#cancel').click(function(){
		...
		if($('#source').val()=="FC")
			{
			  document.location.href=_context+"/bill/frtcorrection/find?shipmentNumber="+$('#shipmentNumber').val()+"&shipmentSequenceNumber="+$('#shipmentSequenceNumber').val()+"&shipmentCorrectionNumber=000&firstPage="+firstPage;
			}
	...
	$("#ediButton").click(function(){
		...
		$.ajax({
			type : "POST",
			url : _context + "/printFreightBill/edi",
			data : {				
	...
	enableEnterEvent();
	tabSequence('#printFreightBillForm',false,false);
	});
...
function singleAddressSelect(){
	console.log("singleAddressSelect");
	$.ajax({
		type : "POST",
...
function updateContactList(){
	...
	$("#printFreightBillForm").attr('action','update');
	$("#printFreightBillForm").attr('method','post');
	$("#printFreightBillForm").ajaxSubmit(options);
...
function showDefaults(responseText, statusText, xhr, $form)  { 
	...
	document.getElementById("printOption").innerHTML="";
	 ...
	 $("#selectedPartyType").get(0).options.length = 0;
	 $('#selectedSC').val(responseText.data.defaultSC);
	 ...
	 $.each(responseText.data.partyTypeList, function(index, partyTypeList) {
	    ...
	    if($("#selectedPartyType").val()=="-1"){
			...
			if(responseText.data.rated) {
				$("#ratedRadio").attr("checked","checked");
				$("#ratedRadio").trigger('change');
				
			} else {
				$("#unRatedRadio").attr("checked","checked");
				$("#unRatedRadio").trigger('change');
			}
			disableUnrated();
			
			
			//$('#ratedRadio').val(responseText.data.noOfCopies);
...
function updateDefaults(){
	...
	$('#printFreightBillForm').attr('action','getDefaultValues');
	blockUI();
	$("#printFreightBillForm").attr('method','post');
	$("#printFreightBillForm").ajaxSubmit(options);
...
function updatePartyList(){
	...
	$('#printFreightBillForm').attr('action','updatePartyList');
	$("#printFreightBillForm").attr('method','post');
	$("#printFreightBillForm").ajaxSubmit(options);
...
function enableDisableButtons(){
	...
	if($('#shipment').val() =="DOMESTIC" && $('#statusCode').val()!="ISSD" &&  $('#statusCode').val()!="CORR") {
		console.log("domestic");
		 /* defect D027318, remove over-ride check */
		 /* defect D027318, changed logic to match spreadsheet, original logic was wrong anyway */
		 $('#saveandsend').attr('disabled','disabled');
	 }
	 else { 
		 console.log("not domestic"); 
		 var selectedRows=$("#contactGrid").jqGrid('getGridParam','selarrrow');
	...
	if($('#shipmentNumber').val().trim()=='' || $('#shipmentSequenceNumber').val().trim()=='') {
		...
		$('#saveandsend').attr("disabled","disabled");
		$('#savecontacts').attr("disabled","disabled");
		$('#sendself').attr("disabled","disabled");
		$('#ediButton').attr("disabled","disabled");
		...
		if($('#ediBtn').val()=="false" || $('#selectedDocType').val()!='FB')  {
			$("#ediButton").attr("disabled","disabled");
		}
		else{
			$("#ediButton").removeAttr('disabled');
		...
		$('#sendself').removeAttr("disabled");
		
		/***************************** Print Button Enable/Disable ******************************/
		
		if($('#printAllowed').val()=="true"){
		...
		/***************************** Print Button Enable/Disable ******************************/
		/***************************** Send from POL Button Enable/Disable ******************************/
		if($('#originPortCityCode').val()=='SHA' || $('#originPortCityCode').val()=='XMN' || $('#originPortCityCode').val()=='NGB'){
			$("#sendpol").removeAttr("disabled");
		}
		else{
			$("#sendpol").attr("disabled","disabled");
		}
	...
	// shipmentHoldReleaseBtn
	// Let hold button control this.
	
	$("#issueBill").attr("disabled","disabled");
	// D031592, disable issue bill btn for EINV bills.
	if($('#statusCode').val()=="RATD" && $('#issueBillBtn').val()=="true"   ) {
		$("#issueBill").removeAttr("disabled");
	}
	
	 
	if($('#statusCode').val()=="ISSD" || $('#statusCode').val()=="CORR") {
		$("#bill").attr("disabled","disabled");
	} else {
		$("#bill").removeAttr("disabled");
	}
	...
	if(responseText.data.selectedDocType=="-1"){
		...
		if(responseText.data.rated==true){
			if(responseText.data.selectedDocType=="FB"){
				// Leaving this here for the disabe but it is redundant to the getDefaults.
				$("#ratedRadio").removeAttr("disabled");
				$("#ratedRadio").attr("checked","checked");
				$("#ratedRadio").trigger('change');
...
function saveandsend(self){
	//D028391: 	SEND BILL DOC - allowed sending of NN without the number of copies
	// not required for FB, NA, DO
	if($('#selectedDocType').val()!="FB" && $('#selectedDocType').val()!="NA" && $('#selectedDocType').val()!="DO"){
		if($('#noOfCopies').val().trim()=="") {
			$("#noOfCopies").validationEngine('showPrompt', 'Please enter no. of copies.', 'error', 'topRight', true);
			return;
		}
		
		if($('#noOfCopies').val()<=0) {
			$("#noOfCopies").validationEngine('showPrompt', 'No. of copies should be greater than zero.', 'error', 'topRight', true);
			return;
		}
	}
	...
	if($('#selectedPartyType').val()=="-1") {
		...
		if(!anySelected && !self){
			...
			console.log("For SavenSend Bill status:"+$('#statusCode').val()+", selectedDocType:"+$('#selectedDocType').val());
			if($('#selectedDocType').val()=="FB" && $('#statusCode').val()!="ISSD" && $('#statusCode').val()!="CORR" && $('#isSaveOnly').val()=='N'){
				var c = confirm("The B/L is not in ISSD/CORR status, are you sure you want to produce this doc?");
				if (c == false) {
					return;
				}
			...
			for(var i=0;i<alreadySelectedContactArray.length;i=i+1){
				...
				if($(alreadySelectedId).is(':checked')){
					...
					isAlreadyPresent=checkForAlreadypresent(selID,selectedContacts);
					if(!isAlreadyPresent){
			...
			if(self && $('#isSaveOnly').val()=='N') {
				$('#printFreightBillForm').attr('action','sendself');
			} else {
				$('#printFreightBillForm').attr('action','savensend');
			}
			$("#printFreightBillForm").attr('method','post');
			blockUI();
			$("#printFreightBillForm").ajaxSubmit(options);
...
// 024870, pass contacts as variable, worried there has been some basic change to chrome that caused this.
function  	checkForAlreadypresent(id,selectedContacts){
	var selID=id;
	var isAlreadyPresent=false;
...
function issueBill(){
	...
	$.ajax({
		async: true,
		type : "POST",
		url : _context + "/printFreightBill/issuebill",
		data : {
			shipment_number :$('#shipmentNumber').val(),
...
function clearContactList(){			
	...
	$('#printFreightBillForm').attr('action','clearList');
	$("#printFreightBillForm").attr('method','get');
	$("#printFreightBillForm").ajaxSubmit(options);	    
}
...
function searchAndPopulateAllContacts(aroleid){		
	$.ajax({
			async: true,
			type : "POST",
			url : _context + "/printFreightBill/searchAndPopulateAllContacts",
			data : {
				aroleid :aroleid,
			},
			success : function(responseText) {
...
function rateBill(){
	...
	var queryString = $("#printFreightBillForm").formSerialize();
	$.ajax({
		type : "POST",
		url : _context + "/printFreightBill/rateBill",
		data : queryString,
		success : function(responseText) {
			$.unblockUI();
			if (responseText.messages.error.length == 0) {
				...
				if(responseText.data.rateView == "showError"){
					$("#printFreightBillForm").loadJSON(responseText.data);
...
//					}
					...
					$("#shipmentNumber").val()+'&shipmentSequenceNumber='+$("#shipmentSequenceNumber").val()
					+'&shipmentCorrectionNumber=000&navigationUrl=printFreightBill&commodityDisplay=last';
				}
...
function concludeRating(id){		
	...
	if($('#reUseSelection').val() != undefined && $('#reUseSelection').val() != ""){
		url = "/printFreightBill/concludeRating?id="+id+"&reUseSelection="+$('#reUseSelection').val();
	}else{
		"/printFreightBill/concludeRating?id="+id;
	}
...
function selectSC() {
	...
	if($('#selectedDocType').val()=="BL"){
		
		$("#selectSC").val("P");
	}
}

function disableUnrated() {
	// D028422
	if($('#selectedDocType').val()=="FB"){
		$("#ratedRadio").attr("checked","checked");
		$("#ratedRadio").trigger('change');
