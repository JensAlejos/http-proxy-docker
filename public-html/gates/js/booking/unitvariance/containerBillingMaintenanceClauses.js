...
$(function() {
	...
	$('#containerClause :input').change(function()
	{
		isClauseChanged = "Y";
		if(this.type=='radio')
		{
	...
	$("#addClauseDialog").dialog({
		...
		buttons:{
			...
			Ok:function()
			{
				if($("#containerClause").validationEngine('validate'))
				{
					/*if($("#standardClauseCode").val()!='BOND')
					{*/
						var queryString = $('#containerClause').formSerialize();
						$.ajax({
							url :  _context +"/containerBilling/addClause",
							type : "POST",
							data : queryString,
							success : function(
									responseText) {
	...
	$('#clausesAdd').click(function() {
		...
		if(isCtrBillingClauseModifiable == true) {
  ...
  var jsonReader = {
		...
		_context+'/containerBilling/loadClauses', // geturl
		'', // addurl
		_context+'/containerBilling/updateClause', // edit url
		_context+'/containerBilling/deleteClause',
		_context+'/containerBilling/deleteClause', // delete selected URL
		...
		$("#gridIdForClauses").jqGrid('setGridParam',{
			delOptions: {url: _context+'/containerBilling/deleteClause', caption: "Confirmation", msg: "Please CONFIRM request to DELETE",
