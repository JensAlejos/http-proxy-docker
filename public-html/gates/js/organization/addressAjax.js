...
$(document).ready(function() { 
	$("#organizationAddressForm").validationEngine('attach'); 	
	...
	/* $('#org_workflow_link').click(function() {
	 ...
	 var i = 0;
    /*$('#organizationAddressForm').submit(function() {
...
function showResponse(responseText, statusText, xhr, $form)  { 
	 ...
	 */
	$('#organizationAddressForm').loadJSON(responseText.data);
	// msgDiv:
	...
	if(responseText.success){
		...
		var ele2 = document.getElementById("noEditNameSave");
	...
	//Comments for ajax resoponse
	
	//set comment and entity id
	$('#organizationAddressId').val(responseText.data.organizationAddressId);
	if($('#organizationAddressId').val() != null && $('#organizationAddressId').val() != "")
	{
		...
		document.organizationAddressForm.addr_delete.disabled=false;
		document.organizationAddressForm.org_address.disabled=false;
		document.organizationAddressForm.add_next_addr.disabled=false;
	}
	else
	{
		//var ele2 = document.getElementById("addHeader");
	...
	//generating div for comment
	var comment1=	"<input type='hidden' id = 'commentId' /><span id ='count'></span>" ;
	var comment2 = "<a class='comments' href='#' ></a>";
	
	//generating div for history
	var hist = "<a class='history' href='#' ></a>";
	
	if($('#organizationAddressId').val()!=null){
		$('#comment_link').html(comment2).append(comment1);
		$('#commentId').val(responseText.data.commentId);
		$('#history_link').html('').append(hist);
	}
