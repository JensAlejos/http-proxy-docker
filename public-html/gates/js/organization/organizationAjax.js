...
$(document).ready(function() { 
    ...
    //on change of organization name get Abbreviation 
    $('#orgName').change(function(){
    	if($('#orgName').val()==''){
    		$('#abbreviation').val('');
    	}else{
    		overrideAction('getAbbreviation');
    ...
    $('#abbreviation').change(function(){
		...
		if($('#abbreviation').val() != null){
			$('#abbreviation').val( $.trim($('#abbreviation').val().toUpperCase()));
		}
		
	});
    $('#org_ap_workflow_link').click(function(){
    	//hide message div
    	if($('#isOrganizationNameDuplicate').val()!="true")
    	{
	    	$("#msgDiv").css('display', 'none');
	    	//get all multi-selected values of org types
	    	submitAllSelect();
	    	//change action
	    	overrideAction('createUpdateOrganization');
	    	//enable fields 
	    	enableFields();
...
function showResponse(responseText, statusText, xhr, $form)  { 
	 ...
	 */
	$('#organizationForm').loadJSON(responseText.data);
